...
7. Use `ida decompile` first and only drop to `ida disassemble` when pseudocode is not
   enough.
8. Record findings with `ida comment`, `ida append-comment`, and `ida rename`.
9. Use `ida patch-bytes` and `ida define-function` when the user explicitly
   wants IDB edits beyond comments and renames.
10. Use `ida py-eval`, `ida py-file`, or raw `ida tool py_exec_file` only when
   the built-in tools are not enough.
11. In WSL, trust the CLI's discovered host candidates instead of hard-coding
   `127.0.0.1`.
12. If the bridge reports `access_mode: local-only`, treat that as expected
